...
<meta name="keywords" content="<?= get_keywords($story['what']) ?>,<?= get_domain($story['url']) ?>" />
...
<div id="main-content">
	...
	<div id="story_<?= $story['id'] ?>"<?= $story_class ?>>
		...
		if(file_exists($_SERVER['DOCUMENT_ROOT'].'/img/'.$category.'/'.$year.'/'.$month.'/'.$day.'/'.$story['headline'].'/')) {
			...
			// Display the images.
			echo '<div class="submit_img">'.display_ext_link($story['url'], image_thumb('/img/'.$category.'/'.$year.'/'.$month.'/'.$day.'/'.$story['headline'].'/'.$files[0]['name'], 70, 70, '', true)).'</div>';
		}
		?>
        <h1>
        ...
        <div class="story-summary">
        	<?= (($story['where'] == '') ? '' : '<span class="story-location">'.$story['where'].' &#151; </span>').display_comment($story['what']) ?> <?= display_ext_link($story['url'], 'Read More') ?>
        </div>
        <div class="story-details">
            Submitted <abbr class="timeago" title="<?= date($this->config->item('date_format'), $story['datesubmitted']) ?>"></abbr> by <a href="/users/<?= $story['username'] ?>"><?= $story['username'] ?></a> via <?= display_ext_link($story['url'], get_domain($story['url'])) ?> 
        ...
        <?php if($user_info['user_is_logged_in'] === true) : ?>
                ...
                <?php endif; ?>
                <?php if(false && $user_info['user_is_admin'] == true) : ?>
    ...
    <?php if(false && !empty($story['posx']) && !empty($story['posy'])) : ?>

...
Mapifies.AddMarker = function ( element, options, callback ) {
	...
	if (options.centerMap) {
		switch (options.centerMoveMethod) {
			case 'normal':
				thisMap.setCenter(new GLatLng(options.pointLatLng[0],options.pointLatLng[1]), options.pointMaxZoom);
			break;
			case 'pan':
	...
	// Add events for start drag
	if(options.dragStart)
		GEvent.addListener(marker, "dragstart", options.dragStart);
	
	// Add events for end drag
	if(options.dragEnd)
			GEvent.addListener(marker, "dragend", options.dragEnd);
 ...
 * @return {Function} callback The callback option with the traffic overlay.
 */
Mapifies.clearOverlays = function ( element, callback ) {
	var thisMap = Mapifies.MapObjects.Get(element);
